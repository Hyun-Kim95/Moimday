...
class _LoginScreenState extends ConsumerState<LoginScreen> {
    ...
    if (!await launchUrl(uri, mode: LaunchMode.externalApplication)) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('ÎßÅÌÅ¨Î•??????ÜÏñ¥??')),
        );
      }
    }
  }
  ...
  Future<void> _signIn(Future<SocialSignInResult> Function() obtain) async {
    if (!_ageOk) {
      setState(() => _error = 'Îß?14???¥ÏÉÅ ?ôÏùòÍ∞Ä ?ÑÏöî?¥Ïöî.');
      return;
    }
    setState(() {
      _loading = true;
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            Text('Í∞ÄÏ°??ºÏ†ï¬∑Î™®ÏûÑ???®Íªò ÎßûÏ∂∞??, style: Theme.of(context).textTheme.bodyMedium),
            ...
            if (kakaoKey.isNotEmpty)
              FtPrimaryButton(
                label: 'Ïπ¥Ïπ¥?§Î°ú ?úÏûë?òÍ∏∞',
                loading: _loading,
                onPressed: _canSignIn ? () => _signIn(social.signInKakao) : null,
              )
            else
              Text(
                'Ïπ¥Ïπ¥?? flutter run --dart-define=KAKAO_NATIVE_APP_KEY=...',
                style: Theme.of(context).textTheme.bodySmall,
              ),
            const SizedBox(height: 8),
            if (googleKey.isNotEmpty)
              FtSecondaryButton(
                label: 'GoogleÎ°??úÏûë?òÍ∏∞',
            ...
            if (defaultTargetPlatform == TargetPlatform.iOS)
              FtSecondaryButton(
                label: 'AppleÎ°??úÏûë?òÍ∏∞',
                onPressed: _canSignIn && !_loading ? () => _signIn(social.signInApple) : null,
              ),
            if (kDebugMode) ...[
              const SizedBox(height: 24),
              Text(
                'Í∞úÎ∞ú: Í∞??úÍ≥µ???§Îäî apps/mobile/docs/oauth-setup.md Ï∞∏Í≥†',
                style: Theme.of(context).textTheme.bodySmall,
              ),
            ],
          ],
